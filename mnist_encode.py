...
## argv[1] - model_type (bgssl, gssl, vae) 
## argv[2] - proportion of training data labeled (300)
## argv[3] - Dataset seed (111)
## argv[4] - thresholdld for mnist reduction (0.1)


model_type, seed = sys.argv[1], int(sys.argv[3])

# Load data
dataset, target = 'mnist', './data/mnist.pkl.gz'
num_labeled, threshold = int(sys.argv[2]), float(sys.argv[4])
...
t = tsne(n_components=2, random_state=0, perplexity=10.0)
...
if model_type == 'bgssl':
    ...
    sslapd_reduced = t.fit_transform(z_sslapd)
...
if model_type == 'gssl':
    ...
    sslpe_reduced = t.fit_transform(z_sslpe)
    print('Done with TSNE transformations...')
...
if model_type == 'vae':
    ...
    vae_reduced = t.fit_transform(z_vae)
    print('Done with TSNE transformations...')
