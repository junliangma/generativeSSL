...
class bgssl:
	...
	for i in range(len(self.NUM_HIDDEN)):
	    ...
	    weights[weight_name] = tf.add(mean_W, tf.multiply(tf.sqrt(tf.exp(logvar_W)), eps_W))
	    weights[bias_name] = tf.add(mean_b, tf.multiply(tf.sqrt(tf.exp(logvar_b)), eps_b))
	mean_W, logvar_W = self.Pzx_y['Wout_mean'], self.Pzx_y['Wout_logvar']
	mean_b, logvar_b = self.Pzx_y['bout_mean'], self.Pzx_y['bout_logvar']
	eps_W = tf.random_normal(mean_W.get_shape(), 0, 1, dtype=tf.float32)
	eps_b = tf.random_normal(mean_b.get_shape(), 0, 1, dtype=tf.float32)
	weights['Wout'] = tf.add(mean_W, tf.multiply(tf.sqrt(tf.exp(logvar_W)), eps_W))
	weights['bout'] = tf.add(mean_b, tf.multiply(tf.sqrt(tf.exp(logvar_b)), eps_b))
	return weights
