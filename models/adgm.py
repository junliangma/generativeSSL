...
class adgm(model):
	...
	z_m, z_lv, z = self.sample_z(x,y,a)
	...
	y_ = tf.tile(tf.expand_dims(y_u,0), [self.mc_samples, 1,1])
        n_u = tf.shape(x)[0]
        lb_u = tf.transpose(tf.reshape(self.lowerBound(x_, y_, z, z_m, z_lv, a_r, qa_m, qa_lv), [self.n_y, n_u]))
	...
	l_qa = dgm.gaussianLogDensity(a, pa_m, pa_lv)
	...
	x_,y_ = tf.tile(tf.expand_dims(x,0), [self.mc_samples,1,1]), tf.tile(tf.expand_dims(y,0), [self.mc_samples,1,1])
        l_qz_in = tf.reshape(tf.concat([x_, y_, a], axis=-1), [-1, self.n_x + self.n_y + self.n_z])
	...
	if a is None:
	    _, _, a = self.sample_a(x)
	qy_in = tf.reshape(tf.concat([tf.tile(tf.expand_dims(x,0), [self.mc_samples,1,1]), a], axis=-1), [-1, self.n_x+self.n_a])
	predictions = dgm.forwardPassCat(qy_in, self.qy_xa, self.n_hid, self.nonlinearity, self.bn, True, 'qy_xa')
