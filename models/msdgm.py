...
""" 
Implementation of modified skip DGMs: p(z) * p(y) * p(a|z,y) * p(x|,y,z,a) * p(y'|x,a)
...
class msdgm(model):
	...
	self.n_a = n_a        # auxiliary variable dimension
   	super(msdgm, self).__init__(n_x, n_y, n_z, n_hid, x_dist, nonlinearity, batchnorm, mc_samples, alpha, l2_reg, ckpt)	

	""" TODO: add any general terms we want to have here """
	self.name = 'modified sdgm'
	...
	self.py_in = tf.concat([self.x_, self.pa_], axis=-1)
	self.py_ = dgm.forwardPassCatLogits(self.py_in, self.py_xa, self.n_hid, self.nonlinearity, self.bn, scope='py_xa', reuse=False)
	self.predictions = self.predict(self.x, training=False)
	self.predictions_q = self.predict_q(self.x, training=False)
	
    def compute_loss(self):
	""" manipulate computed components and compute loss """
	elbo_l, qy_l = self.labeled_loss(self.x_l, self.y_l)
	self.elbo_l, self.qy_l = -tf.reduce_mean(elbo_l), -tf.reduce_mean(qy_l)/self.alpha	
	...
	l_py_prime = self.compute_logpy(y,x,a)
	...
	return tf.reduce_mean(l_px + l_py + l_py_prime + l_pz + l_pa - l_qz - l_qa, axis=0)	
    ...
    def compute_logpy(self, y, x, a):
	""" compute logp(y|x,a) for y' """
	py_in = tf.reshape(tf.concat([x, a], axis=-1), [-1, self.n_x+self.n_a])
	y_ = tf.reshape(dgm.forwardPassCatLogits(py_in, self.py_xa, self.n_hid, self.nonlinearity, self.bn, True, 'py_xa'), [self.mc_samples,-1,self.n_y])
        return dgm.multinoulliLogDensity(y, y_)
    ...
    def predict_q(self, x, a=None, training=True):
	""" predict y for given x with q(y|x,a) """
	if a is None:
	    _, _, a = self.sample_a(x)
	...
	if self.x_dist == 'Gaussian':
    	...
    	self.py_xa = dgm.initCatNet(self.n_x+self.n_a, self.n_hid, self.n_y, 'py_xa_')
    ...
    def print_verbose1(self, epoch, fd, sess):
		...
		" Training (p): {:5.3f}, Testing (p): {:5.3f}".format(epoch, total, elbo_l, elbo_u, qy, train_acc, test_acc))
    ...
    def print_verbose2(self, epoch, fd, sess):
		...
		" Training: {:5.3f}, Teisting: {:5.3f}".format(epoch, elbo_l, qy, self.alpha, weights/self.n_train, train_acc, test_acc)) 

    def compute_accuracies(self):
        self.train_acc = self.compute_acc(self.x_train, self.y_train)
        self.test_acc = self.compute_acc(self.x_test, self.y_test)
        self.train_acc_q = self.compute_acc_q(self.x_train, self.y_train)
        self.test_acc_q = self.compute_acc_q(self.x_test, self.y_test)

    def predict_q_new(self, x):
        saver = tf.train.Saver()
        with tf.Session() as session:
            ckpt = tf.train.get_checkpoint_state(self.ckpt_dir)
            saver.restore(session, ckpt.model_checkpoint_path)
            preds = session.run(self.predictions_q, {self.x:x})
        return preds	
