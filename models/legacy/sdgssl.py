...
""" 
Generative models for labels with stochastic inputs and auxiliary variable: p(z) * p(a|z) * p(x|a,z) * p(y|x,a,z) 
...
class sdgssl(model):
	...
	super(sdgssl, self).__init__(Z_DIM, LEARNING_RATE, NUM_HIDDEN, TYPE_PX, NONLINEARITY, BATCHNORM, temperature_epochs, start_temp, NUM_EPOCHS, Z_SAMPLES, BINARIZE, logging, eval_samps=eval_samps, ckpt=ckpt)
	...
	for i in range(n_iters):
	    ...
	    h = tf.concat([x, a, z], axis=1)
	    y_ = dgm._forward_pass_Cat(h, self.Pzax_y, self.NUM_HIDDEN, self.NONLINEARITY, self.batchnorm, self.phase)
	...
	pa_mean, pa_log_var = dgm._forward_pass_Gauss(z, self.Pz_a, self.NUM_HIDDEN, self.NONLINEARITY, self.batchnorm, self.phase)
	l_px = self._compute_logpx(x, a, z)
	l_py = self._compute_logpy(y, x, z, a)
    ...
    def _compute_logpx(self, x, a, z, y=None):
        """ compute the likelihood of every element in x under p(x|z,a) """
	h = tf.concat([a,z], axis=1)
        if self.TYPE_PX == 'Gaussian':
            mean, log_var = dgm._forward_pass_Gauss(h, self.Pza_x, self.NUM_HIDDEN, self.NONLINEARITY, self.batchnorm, self.phase)
            return dgm._gauss_logp(x, mean, log_var)
        elif self.TYPE_PX == 'Bernoulli':
            logits = dgm._forward_pass_Cat_logits(h, self.Pza_x, self.NUM_HIDDEN, self.NONLINEARITY, self.batchnorm, self.phase)
            return -tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=x, logits=logits),axis=1)

    def _compute_logpy(self, y, x, z, a):
	""" compute the likelihood of every element in y under p(y|x,z) """
	h = tf.concat([x, a, z], axis=1)
	y_ = dgm._forward_pass_Cat_logits(h, self.Pzax_y, self.NUM_HIDDEN, self.NONLINEARITY, self.batchnorm, self.phase)
	return -tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=y_)
    ...
    def _print_verbose1(self,epoch, fd, sess, acc_train, acc_test, avg_var=None):
        ...
        lpx_test, lpx_train, klz_test, klz_train = sess.run([self._compute_logpx(self.x_test, a_test, z_test),
                                                                  self._compute_logpx(self.x_train, a_train, z_train),
