...
Inference network: q(a,y,z|x) = q(y|x) * q(a|x,y) * q(z|a,y,x)
"""
...
class agssl(model):
    ...
    def _sample_a(self, x, y, n_samples):
	""" Sample from a with the reparamterization trick """
	h = tf.concat([x, y], axis=1)
	mean, log_var = dgm._forward_pass_Gauss(h, self.Qxy_a, self.NUM_HIDDEN, self.NONLINEARITY, self.batchnorm, self.phase)
	...
	qa_mean, qa_log_var, a = self._sample_a(x, y, self.Z_SAMPLES)
	...
	self.Pzxy_a = dgm._init_Gauss_net(self.Z_DIM+self.X_DIM+self.NUM_CLASSES, self.NUM_HIDDEN, self.A_DIM, 'P_A_', self.batchnorm) 
    	...
    	self.Qxy_a = dgm._init_Gauss_net(self.X_DIM+self.NUM_CLASSES, self.NUM_HIDDEN, self.A_DIM, 'Qxy_a_', self.batchnorm)
    ...
    def _print_verbose1(self,epoch, fd, sess, acc_train, acc_test, avg_var=None):
        am_test, alv_test, a_test = self._sample_a(self.x_test, self.y_test, 1)
        am_train, alv_train, a_train = self._sample_a(self.x_train, self.y_train, 1)
